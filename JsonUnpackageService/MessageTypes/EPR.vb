...
Public Class EPR
    ...
    Public Overrides Function GetSqlStatement() As String
        ...
        output += $"'{Payment_comment}','{GetJsonIndex}');"
        output = output.Replace("''", "null")

        Select Case UI_Type
            Case AggregationType.Unit_Packets_Only
                output += $"UPDATE `{DBBase.DBName}`.`tblprimarycodes` SET fldEPR = '{GetJsonIndex}' "
                output += $"WHERE fldPrintCode in ('{String.Join("','", upUIs)}');"
            Case AggregationType.Aggregated_Only
                output += $"UPDATE `{DBBase.DBName}`.`tblaggregatedcodes` SET fldEPR = '{GetJsonIndex}' "
            ...
            Case AggregationType.Both
                ...
                output += $"SET P.fldEPR = '{GetJsonIndex}', A.fldEPR = '{GetJsonIndex}' "
